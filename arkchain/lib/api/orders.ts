...
export async function getOpenOrders(companyId?: string): Promise<Order[]> {
  ...
  // Sequential awaits so TypeScript correctly infers each log type
  const postedLogs = await fujiClient.getLogs({
  ...
  const cancelledLogs = await fujiClient.getLogs({
    address: CONTRACT_ADDRESSES.OrderBook,
  ...
  const executedLogs = await fujiClient.getLogs({
    address: CONTRACT_ADDRESSES.OrderBook,
  ...
  for (const log of postedLogs) {
    ...
    // pricePerToken stored as USDC with 6 decimals; convert to MXN
    const pricePerTokenMXN =
      (Number(pricePerToken ?? BigInt(0)) / 1_000_000) * MXN_PER_USDC;
    const tokensForSale = amount ?? BigInt(0);
